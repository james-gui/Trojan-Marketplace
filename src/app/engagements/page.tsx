...
import { DollarSign, ShieldCheck, Check, Loader2, ChevronDown, ChevronUp, CheckCircle } from "lucide-react";
...
function ListingCard({
    ...
    return (
        ...
        >
            ...
            {listing.proofImageUrl && listing.aiVerificationStatus && (
                <div className={`text-xs px-2 py-1 mt-2 rounded inline-flex items-center gap-1 ${listing.aiVerificationStatus === 'Verified' ? 'bg-green-100 text-green-800' :
                    listing.aiVerificationStatus === 'Failed' ? 'bg-red-100 text-red-800' :
                        'bg-amber-100 text-amber-800'
                    }`}>
                    {listing.aiVerificationStatus === 'Verified' ? <CheckCircle size={12} /> :
                        listing.aiVerificationStatus === 'Failed' ? <ShieldCheck size={12} /> :
                            <Loader2 size={12} className="animate-spin" />}
                    AI Verification: {listing.aiVerificationStatus}
                </div>
            )}
    ...
    return (
        <div className="space-y-6">
                ...
                task={
                    selectedTask
                        ? {
                            ...
                            price: `$${Number(selectedTask.price).toFixed(2)}`,
                            description: selectedTask.description,
                            postedBy: selectedTask.posterName || selectedTask.posterEmail,
                            timeAgo: formatTimeAgo(selectedTask.createdAt) || "Recent",
            ...
            <ProofModal
                ...
                aiVerificationStatus={proofTask?.aiVerificationStatus}
                aiVerificationReason={proofTask?.aiVerificationReason}
                role={
